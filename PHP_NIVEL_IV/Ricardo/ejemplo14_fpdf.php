...
class PDF extends FPDF{
    ...
    function Header()
    {
        ...
        $this->Cell(80);
        // Título
        $texto = mb_convert_encoding('LISTADO DE ESTUDIANTES DE PHP NIVEL IV', 'ISO-8859-1', 'UTF-8');
     ...
     */
    $estudiantes = file('../datos/estudiantes.txt');
    //var_dump($estudiantes);
    //echo "<hr>";
    for ($i=0; $i < count($estudiantes); $i++) { 
        // Picar el contenido de cada posición
        $detalle = explode(";", $estudiantes[$i]);
        //var_dump($detalle);
        //echo "<hr>";
        $nombre = $detalle[0];
        $apellido = $detalle[1];
        $texto_original = $nombre . ", " . $apellido; 
        $texto = mb_convert_encoding($texto_original, 'ISO-8859-1', 'UTF-8');
