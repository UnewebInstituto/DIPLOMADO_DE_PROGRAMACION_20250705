<?php
    ...
    try {
        ...
        if (mysqli_errno($enlace) == 1062){
            $mensaje = "Datos personales no se almacenaron debido a que la cédula " .$cedula ." o el correo electrónico " .$_POST['correoelectronico'] .  " ya se encuentran registrados.";
            $severidad = 3;
        }else{
            $mensaje = "Datos personales no fueron almacenados, Código: " . mysqli_errno($enlace) . ", Detalle: " . $th->getMessage();
            $severidad = 2;
        }
    } finally {
